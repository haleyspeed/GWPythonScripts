...
def get_n (data):
	...
	for index, row in data.iterrows():
		...
		if treatment1 in row[factor1] and row[factor2] == 0:
			n1 = n1 + 1
		elif treatment2 in row[factor1] and row[factor2] == 0:
			n2 = n2 + 1
		elif treatment3 in row[factor1] and row[factor2] == 0:
			n3 = n3 + 1  
		elif treatment4 in row[factor1] and row[factor2] == 0:
			n4 = n4 + 1  
...
slice_stats = get_desc (data, factor1, factor2, measurement1, measurement1_std, measurement1_ste, measurement2, measurement2_std, measurement2_ste)

# Calculates data per mouse
per_mouse = data.groupby(['mouse_id',factor1, factor2]).mean()
per_mouse = per_mouse.reset_index()
per_mouse = per_mouse[['mouse_id', factor1, 'n', factor2, measurement2, measurement1]] 

# Calculates n per mouse
get_n (per_mouse)

# Calculates stats per mouse
mouse_stats = get_desc (per_mouse, factor1, factor2, measurement1, measurement1_std, measurement1_ste, measurement2, measurement2_std, measurement2_ste)
